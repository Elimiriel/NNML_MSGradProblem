...
import torch
from .phyparameters import xdist, ywidth, rAdS, G_N, ensure_tensordotdim
from process_runtime.precperfs import realTType, vErr


class Reference():
    def __init__(self, targetmodel: Literal["linear-axion", "gubser-rocha"], z, beta, mu):
        """_summary_
        ...
        if targetmodel == "linear-axion":
            self.Q=None
        elif targetmodel == "gubser-rocha":
            self.Q=self._resQ(self.mu, self.beta)
            if self.Q.dim()==1:
                self.Q=self.Q.unsqueeze(0)
        else:
            raise NotImplementedError(f"Unrecognized targetmodel: {targetmodel}")
